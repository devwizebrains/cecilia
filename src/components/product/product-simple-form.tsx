...
export default function ProductSimpleForm({ initialValues }: IProps) {
  ...
  return (
    <div className="my-5 flex flex-wrap sm:my-8 getForm">
      <Description
      ...
      <Card className="w-full sm:w-8/12 md:w-2/3">
        <div className="mb-5">
          <Label>{t('Select Product Price')}</Label>
          <SelectInput
                name="productprice"
                control={control}
                ...
                onChange={(selectedOption) => {
                  if (selectedOption && selectedOption.value === 'fixed_price') {
                    setIsPriceOptional(true);
                  } else {
                    setIsPriceOptional(false);
                  }
                }}
              />
        </div>
        <Input 
          label={`${t('form:input-label-price')}${isPriceOptional ? '' : '*'}`}
          {...register('price')}
          type="number"
